...
**AI Engineer YouTube** — do **not** `open_page` the `/videos` tab (JS-walled). Run `bash scripts/fetch-youtube.sh 14` from this skill folder (RSS). Keep 3–6 dated titles + URLs in window. Do not transcribe talks. A title can amplify a cluster; it is not a primary source for a platform change.
...
Wait up to 120s. Strip grok preamble. Then still run LinkedIn `web_search` and the YouTube + GitHub scripts yourself. Do not nest `grok -p` when X tools already exist.
...
## Shopify
Deadlines first. Then official blog + changelog entries in-window. For each: what changed, who it hits, date, URL.

## AI
Models, agents, infra. One tight paragraph each, not a headline-only list.

## E-commerce (Shopify-shaped)
What Shopify is focusing on (Catalog, UCP, agentic storefronts, WebMCP). Skip generic Amazon-vs-TikTok evergreen.

## Launches
- **Name** — what it is, who shipped it, why a Shopify/AI/ecom team might care. Link.

## GitHub
- **owner/repo (★, date)** — one sentence. URL. Prefer org:Shopify and new high-star agent/MCP repos.

## AI Engineer
- **YYYY-MM-DD · Title** — one clause on the cluster it maps to. [watch](https://youtube.com/watch?v=…)
